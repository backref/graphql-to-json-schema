...
async function handler(source, flags) {
  try {
    ...
    if (!outputDir) {
      throw new Error('Multiple files found, --output-dir must be a valid directory')
    }
...
async function processFile(graphqlFilename, outputFilename, {dryRun, printHeader}) {
  try {
...
async function tranformGraphqQLFile(filename) {
  ...
  return g2j.parseGraphQL(schemaText, filename)
